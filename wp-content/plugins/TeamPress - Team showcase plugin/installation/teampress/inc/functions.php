	...
	if($first_char!=''){
		...
		if(!empty($postids)){
			if($ids !=''){
				...
				if(empty($arr)){
					$html = esc_html__('No matching records found','teampress');
...
if(!function_exists('extp_search_form_html')){
	function extp_search_form_html($cats,$category_style){
        ...
        <form role="search" method="get" id="searchform" class="etp-search-form <?php echo $class; ?>" action="<?php echo home_url(); ?>/">
            ...
            <?php if ( ! empty( $terms ) && ! is_wp_error( $terms ) ){ ?>
            	<?php if ($category_style=='inline'){ 
            		extp_child_cat_html($cats);
            	} ?>
                <select name="extp_cat">
                	<option value=""><?php echo get_option('we_text_all')!='' ? get_option('we_text_all') : esc_html__('All','teampress'); ?></option>
                  	<?php
                  	$count_stop = 5;
				  	foreach ( $terms as $term ) {
				  		echo '<option value="'. $term->slug .'">'. $term->name .'</option>';
				  		extp_show_child_inline($cats,$term,$count_stop,'');
				  	}
				  ?>
                </select>
            <?php
...
if(!function_exists('extp_child_cat_html')){
	function extp_child_cat_html($cats){
		$args = array(
			'parent'        => '0',
			'hide_empty'        => true,
		);
        ...
        <div class="extp-child_cat">
			<ul class="extp-top-cat">
				<?php
				echo '<li class="extp-top-term extp-child-click extp-child-active" data-value=""><span>'.esc_html__('All','teampress').'</span>';
				
				foreach ($terms as $term) {
					echo '<li class="extp-top-term extp-child-click " data-value="'.$term->slug.'">'.$term->name;
					extp_show_child_inline($cats,$term,$count_stop,'inline');
...
if(!function_exists('extp_show_child_inline')){
	function extp_show_child_inline($cats,$term,$count_stop,$inline){
		if ($count_stop < 2) {
			return;
		}
		$charactor ='';
		...
		if ($second_level_terms) {
			...
			if ($inline != 'inline') {
				foreach ($second_level_terms as $second_level_term) {
					...
					extp_show_child_inline($cats,$second_level_term,$count_stop,'');
				}
			}else{
		        ...
		        foreach ($second_level_terms as $second_level_term) {
		            ...
		            echo '<li class="extp-child-click" data-value="'.$second_level_term->slug.'">'.$second_term_name;
		            extp_show_child_inline($cats,$second_level_term,$count_stop,'inline');
		            echo '</li>';
		        }
