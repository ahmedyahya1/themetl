...
extract( shortcode_atts( array(
	...
	'button_google_fonts'		=> '',
	'button_custom_fonts'		=> '',
	'list_border_color'			=> '#ececec',
	'month_day_font_options'	=> '',
	'month_day_google_fonts'	=> '',
	'month_day_custom_fonts'	=> '',
	'name_location_font_options'=> '',
	'name_location_google_fonts'=> '',
	'name_location_custom_fonts'=> '',
	'hover_text_color'			=> '',
	'class'						=> '',
...
if(substr_count($box_hover_shadow, 'disable') == 0) {
	...
	$zolo_event_list_id = 'zolo_event_list_element_'.$uniqid;
...
<?php
	...
	//Typo
	$month_day_options = _zolo_parse_text_shortcode_params($month_day_font_options, '', $month_day_google_fonts, $month_day_custom_fonts);
	$name_location_options = _zolo_parse_text_shortcode_params($name_location_font_options, '', $name_location_google_fonts, $name_location_custom_fonts);
	...
	$output = '<div id="'.$zolo_event_list_id.'" class="zolo_event_list_element '.$animatedclass.' '.$class.'" data-animation = "'.$data_animation.'" data-delay = "'.$data_delay.'">';
	
	//event_list_wrapper start
	$output .= '<div class="zolo_event_list_wrapper">';
	$output .= '<div class="zolo_event_list">';
	
	$output .= '<div class="zolo_event_timeframe" ' . $month_day_options['style'] . '><div class="zolo_event_date">'.esc_html($date_text).'</div><div class="zolo_event_day">'.esc_html($day_text).'</div></div>';
	
	if ( $use_button_link ) {
		
    	$output .= '<div class="zolo_event_venue"><a ' . $attributes . ' ' . $name_location_options['style'] . '>'.esc_html($venue_text).'</a></div>';
		$output .= '<div class="zolo_event_location"><a ' . $attributes . ' ' . $name_location_options['style'] . '>'.esc_html($location_text).'</a></div>';
		
	}else{
		
		$output .= '<div class="zolo_event_venue" ' . $name_location_options['style'] . '>'.esc_html($venue_text).'</div>';
		$output .= '<div class="zolo_event_location" ' . $name_location_options['style'] . '>'.esc_html($location_text).'</div>';
		}
	
	// Button Start
	$output .= '<div id="'.$zolo_button_id.'" class="zolo_button_element '.$button_wrap_class.' '.$color_scheme.'">';
	if ( $use_button_link ) {
...
$shortcode_css .= '#'.$zolo_event_list_id.'.zolo_event_list_element:before{ content:""; top:0; left:0; bottom:0; right:0; position:absolute; background:url('.$image.') no-repeat center center; -moz-background-size:cover; -webkit-background-size:cover;-o-background-size:cover; -ms-background-size:cover; background-size:cover; transition:all 0.6s;-moz-transition:all 0.6s;-webkit-transition:all 0.6s;-ms-transition:all 0.6s;-o-transition:all 0.6s; opacity:0;}';
$shortcode_css .= '#'.$zolo_event_list_id.'.zolo_event_list_element:hover:before{ opacity:1;}';
$shortcode_css .= '#'.$zolo_event_list_id.'.zolo_event_list_element .zolo_event_list{ border-color:'.$list_border_color.';}';
...
#'.$zolo_button_id.' .zolo_button:hover{'.$color_scheme_css.'}';
}
...
$shortcode_css .= '#'.$zolo_event_list_id.'.zolo_event_list_element .zolo_event_list:hover .zolo_event_venue,
#'.$zolo_event_list_id.'.zolo_event_list_element .zolo_event_list:hover .zolo_event_location,
#'.$zolo_event_list_id.'.zolo_event_list_element .zolo_event_list:hover .zolo_event_timeframe .zolo_event_date, 
#'.$zolo_event_list_id.'.zolo_event_list_element .zolo_event_list:hover .zolo_event_timeframe .zolo_event_day{ color:'.$hover_text_color.'!important;}';
