...
/**
 * Class Apress_Vc_Templates_Panel_Editor
 * @since 1.0
 */
class Apress_Vc_Templates_Panel_Editor {
	/**
	 ...
	 * @var bool
	 */
	protected $apress_templates = false;
	/**
	 * @since 4.4
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'apress_templates',
			'category_name'   => esc_html__( 'Apress Gallery', 'apcore' ),
			'category_weight' => 1,
	...
	protected function get_template_categories() {
		...
		$categories = array(
			'all'					=> esc_html__( 'All', 'apcore' ),
			'generals'				=> esc_html__( 'General', 'apcore' ),
			'about'					=> esc_html__( 'About', 'apcore' ),
			'blog'					=> esc_html__( 'Blog', 'apcore'),			
			'call-to-action'		=> esc_html__( 'Call to Action', 'apcore' ),
			'case-studies'			=> esc_html__( 'Case Studies', 'apcore' ),	
			'contact'				=> esc_html__( 'Contact', 'apcore' ),
			'counters'				=> esc_html__( 'Counters', 'apcore' ),		
			'ecommerce'				=> esc_html__( 'Ecommerce', 'apcore' ),	
			'faqs'					=> esc_html__( 'Faqs', 'apcore' ),	
			'features'				=> esc_html__( 'Features', 'apcore' ),	
			'hero-sections'      	=> esc_html__( 'Hero Sections', 'apcore' ),							
			'icon-box'				=> esc_html__( 'Icon Boxes', 'apcore' ),			
			'map'					=> esc_html__( 'Maps', 'apcore' ),
			'media'					=> esc_html__( 'Media', 'apcore' ),
			'portfolio'				=> esc_html__( 'Portfolio', 'apcore' ),	
			'pricing'				=> esc_html__( 'Pricing', 'apcore' ),
			'process'				=> esc_html__( 'Process', 'apcore' ),
			'service'				=> esc_html__( 'Services', 'apcore' ),		
			'tab'					=> esc_html__( 'Tabs', 'apcore' ),	
			'team'					=> esc_html__( 'Teams', 'apcore' ),
			'testimonial'			=> esc_html__( 'Testimonials', 'apcore' ),	
		);
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'apress_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 apress-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-sm-12 apress-templates-container">
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'apress_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowApressTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowApressTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'apcore' );
		$add_template_title = esc_attr( 'Add template', 'apcore' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_Apress_Templates::renderBackendTemplate:1' );
		}
	 ...
	 * @param $templates
	 *
	 * vc_filter: vc_load_apress_templates_limit_total - total items to show
	 *
	 * @return array
	 */
	...
	public function getAllTemplates() {
		...
		$data = array();
		$apress_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $apress_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'type' => 'apress_templates',
	 ...
	 * To modify you should use add_filter('vc_load_apress_templates','your_custom_function');
	 ...
	 *          ...
	 *      );
	 * Also see filters 'vc_load_apress_templates_panels' and 'vc_load_apress_templates_welcome_block' to modify
	 * templates in panels tab and/or in welcome block. vc_filter: vc_load_apress_templates - filter to override
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->apress_templates ) ) {
			$this->apress_templates = $this->allTemplates();
		}

		return $this->apress_templates;
	}
	...
	public function getDefaultTemplate( $template_index ) {
		...
		if ( ! is_numeric( $template_index ) || ! is_array( $this->apress_templates ) || ! isset( $this->apress_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->apress_templates[ $template_index ];
	}
	...
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->apress_templates ) ) {
				$this->apress_templates = array();
			}
			$this->apress_templates[] = $data;
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'apcore' ) );
...
HTML;
		$output .= '<div class="apress-template-preview"><img src="' . esc_url( $template_image ) . '" alt="' . esc_attr( $name ) . '" width="300" height="200" /></div>';
		$output .= apply_filters( 'vc_templates_render_template', $name, $template );
		$output .= '<span class="apress-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
